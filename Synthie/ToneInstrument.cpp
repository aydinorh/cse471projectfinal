...
void CToneInstrument::SetNote(CNote *note)
{
	...
	for (int i = 0; i<len; i++)
	{
		...
		else if (name == "threshold")  // New addition
		{
			value.ChangeType(VT_R8);
			m_sinewave.SetThreshold(value.dblVal);  // Set noise gate threshold
		}
		else if (name == "compressionThreshold")
		{
			value.ChangeType(VT_R8);
			m_sinewave.SetCompressionThreshold(value.dblVal);
		}
		else if (name == "compressionRatio")
		{
			value.ChangeType(VT_R8);
			m_sinewave.SetCompressionRatio(value.dblVal);
		}
	}
